  ...
  class GetBalanceNotificationSettingsResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TextmagicRestClient::GetBalanceNotificationSettingsResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TextmagicRestClient::GetBalanceNotificationSettingsResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'low_balance_notification')
        self.low_balance_notification = attributes[:'low_balance_notification']
      else
        self.low_balance_notification = nil
      end

      if attributes.key?(:'alert_balance')
        self.alert_balance = attributes[:'alert_balance']
      else
        self.alert_balance = nil
      end

      if attributes.key?(:'alert_phone')
        self.alert_phone = attributes[:'alert_phone']
      else
        self.alert_phone = nil
      end
